...
for k = 1:n_seg
    ...
    scatter(poly_coef_x((k-1) * (n_order + 1) + 1:k * (n_order + 1)), poly_coef_y((k-1) * (n_order + 1) + 1:k * (n_order + 1)), color(k), 'filled', 'LineWidth', 2);
    plot(x_pos(startidx:idx-1), y_pos(startidx:idx-1), color(k), 'linewidth', 2);
end
...
function poly_coef = MinimumSnapCorridorBezierSolver(axis, waypoints, corridor, ts, n_seg, n_order, v_max, a_max)
    ...
    %                                   ...,
    %                                   pn_min, pn_max ];
%     corridor_range_tmep = [corridor(axis, 1) - corridor(axis + 2, 1), corridor(axis, 1) + corridor(axis + 2, 1)];
